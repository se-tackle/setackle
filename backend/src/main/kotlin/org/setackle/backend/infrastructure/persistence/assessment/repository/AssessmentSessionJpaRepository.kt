...
import org.setackle.backend.infrastructure.persistence.assessment.entity.AssessmentSessionJpaEntity
...
interface AssessmentSessionJpaRepository : JpaRepository<AssessmentSessionJpaEntity, Long> {
    ...
    fun findByUserIdAndSkillId(
        @Param("userId") userId: Long,
        @Param("skillId") skillId: Long,
    ): List<AssessmentSessionJpaEntity>
    ...
    @Query(
        """
        SELECT a FROM AssessmentSessionJpaEntity a
        WHERE a.expiredAt < :now AND a.status IN ('CREATED', 'IN_PROGRESS', 'PAUSED')
        """,
    )
    fun findExpiredActiveSessions(
